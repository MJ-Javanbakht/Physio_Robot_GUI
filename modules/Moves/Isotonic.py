...
from modules.canOpen.config import init_drive
...
class Isotonic:
    def __init__(self):
        ...
        self.force2torque = 29.55 / 100  # ???
        self.torque2voltage = 10 / 31.8  # ???
    ...
    def safetyFunc(self, theta, thetaSatPositive, thetaSatNegative, velocity, forceRef):
        # Implementation of safetyFunc...
        upperLimit = (7) * 15
        lowerLimit = (-7) * 15
        Ir = (
            (
                self.m * self.g * math.sin((thetaSatPositive - 10) * self.deg2rad)
                + forceRef
            )
            * self.force2torque
            * self.torque2voltage
        ...
        Irmin_p = (
            (
                self.m * self.g * math.sin((-thetaSatNegative + 10) * self.deg2rad)
                + forceRef
            )
            * self.force2torque
            * self.torque2voltage
        ...
        Irmin_n = (
            (
                self.m * self.g * math.sin((-thetaSatNegative + 20) * self.deg2rad)
                + forceRef
            )
            * self.force2torque
            * self.torque2voltage
        ...
        if forceRef >= 0:
            ...
            if (theta > thetaSatPositive - 10) and (theta < thetaSatPositive):  # 1
                ...
                output = 0
            # Near ROM (10)
            elif (theta > ((-thetaSatNegative) * (-1))) and (
                theta < ((-thetaSatNegative) * (-1)) + 10
            ):  # 3
                output = (
                    forceRef  # (((Irmin_p - Irmax)/10)*(theta+thetaSatNegative))+Irmax
                )
    ...
    def forceCompensator(self, theta, velocity):
        # Implementation of forceCompensator...
        if theta < 0:
            if velocity < 0:
                output = (
                    0.0084924 * (velocity * 0.0000002094)
                    + 0.0043534 * math.sin(theta * 0.0000002094)
                    + 24.0742 * -1
                )
    ...
    def weightCompensator(self, theta):
        # Implementation of weightCompensator...
        output = self.m * self.g * math.sin(self.deg2rad * theta)
        return output

    # end def

    def force2voltageFunc(self, force):
        # Implementation of force2voltageFunc...
        output = force * self.force2torque * self.torque2voltage
        return output

    # end def

    def controllerSwitch(self, theta, time, fromSafety):
        # Implementation of controllerSwitch...
    ...
    def run(
        ...
        try:
            while set_i < setsDesired:
                theta = node.tpdo[1]["Position actual value"].raw * (
                    360 / (1280000 * 25)
                )
                ...
                out_safetyFunc = self.safetyFunc(
                    theta, thetaSatPositive, thetaSatNegative, vel, force_setRep
                )
                ...
                print(
                    ...
                    + str(force_setRep)
                    + ", safety : "
                    + str(int(out_safetyFunc))
                    + ", set : "
                    + str(set_i),
                    "repeats : " + str(repeats),
                )  # + ', TCommand : ' + str(torqueCommand))
                # str(force_setRep))
                # time.sleep(0.005)
